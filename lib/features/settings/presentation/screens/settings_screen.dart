...
class SettingsScreen extends StatelessWidget {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // AI Settings Section
              _buildSectionHeader('ai_model'.tr),
              Card(
                child: Obx(() {
                  // Listen to update trigger
                  storageService.updateTrigger.value;
                  
                  final selectedId = storageService.getSelectedGeminiModel();
                  final model = GeminiService.availableModels.firstWhere(
                    (m) => m.id == selectedId,
                    orElse: () => GeminiService.availableModels.first,
                  );
                  final usage = storageService.getModelUsage(model.id);
                  final remaining = model.dailyLimit - usage;
                  ...
                  return ListTile(
                    ...
                    subtitle: Text(
                      isExhausted 
                        ? 'model_exhausted'.tr 
                        : 'remaining_requests'.tr.replaceAll('{count}', remaining.toString()),
                      style: TextStyle(
                        color: isExhausted ? Colors.red : Colors.grey,
                      ),
                    ),
                    trailing: const Icon(Icons.arrow_forward_ios, size: 16),
                    onTap: () => _showModelDialog(context, storageService),
                  );
                }),
              ),
  ...
  void _showModelDialog(BuildContext context, StorageService storageService) {
    Get.bottomSheet(
      Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
                ...
                children: GeminiService.availableModels.map((model) {
                  ...
                  return ListTile(
                    ...
                    trailing: isSelected 
                        ...
                        : null,
                    onTap: () async {
                      await storageService.setSelectedGeminiModel(model.id);
                      storageService.triggerUpdate();
                      Get.back();
                    },
                  );
                }).toList(),
